...
Select 
location, Max(Cast(total_deaths as signed)) highest_death_count 
from PortfolioProject.coviddeaths 
-- Where location like '%States%'
Where Trim(continent) <> ''
Group by location
...
Select 
date, Sum(new_cases) as global_cases, Sum(Cast(new_deaths as signed)) as global_deaths, Sum(Cast(new_deaths as signed))/Sum(new_cases)*100 as global_death_percentage
from PortfolioProject.coviddeaths 
Where Trim(continent) <> ''
Group by date
Order by 1;
...
Select dea.continent, dea.location, dea.date, dea.population, Cast(Nullif(Trim(vac.new_vaccinations), '') as signed), 
Sum(Cast(Nullif(Trim(vac.new_vaccinations), '') as signed))  over (Partition by dea.location Order by dea.date) as rolling_people_vaccinated 
-- (rolling_people_vaccinated/population)*100 
from PortfolioProject.coviddeaths dea
Join PortfolioProject.covidvaccinations vac
	on dea.location = vac.location and dea.date = vac.date
...
Drop view if exists PortfolioProject.PercentPeopleVaccinated;
